...
init()
{
    register_clientfield_alt( "scriptmover", "perk_bottle_cycle_state", "int", ::start_bottle_cycling_alt );
	register_clientfield_alt( "scriptmover", "turn_active_perk_light_red", "int", ::turn_on_active_light_red_alt );
	register_clientfield_alt( "scriptmover", "turn_active_perk_light_green", "int", ::turn_on_active_light_green_alt );
	register_clientfield_alt( "scriptmover", "turn_on_location_indicator", "int", ::turn_on_location_indicator_alt );
	register_clientfield_alt( "scriptmover", "turn_active_perk_ball_light", "int", ::turn_on_active_ball_light_alt );
	register_clientfield_alt( "scriptmover", "zone_captured", "int", ::zone_captured_cb_alt );
...
turn_on_location_indicator_alt( new_val )
{
    if ( !isdefined( self.turn_on_location_indicator_alt_old_val ) )
    {
        self.turn_on_location_indicator_alt_old_val = 0;
	}
	oldval = self.turn_on_location_indicator_alt_old_val;
	newval = int( new_val );
    if ( newval == 1 )
        self thread fx_location_indicator( 0 );
    else
    {
        self notify( "ball_departed" );
        self thread fx_departure_steam( 0 );
    }
}
...
zone_captured_cb_alt( new_val )
{
    if ( !isdefined( self.zone_captured_cb_alt_old_val ) )
    {
        self.zone_captured_cb_alt_old_val = 0;
	}
	oldval = self.zone_captured_cb_alt_old_val;
	newval = int( new_val );
    if ( !isdefined( self.mapped_const ) )
    {
        self mapshaderconstant( 0, 1, "ScriptVector0" );
        self.mapped_const = 1;
    }
...
turn_on_active_light_red_alt( new_val )
{
    if ( !isdefined( self.turn_on_active_light_red_alt_old_val ) )
	{
		self.turn_on_active_light_red_alt_old_val = 0;
	}
	oldval = self.turn_on_active_light_red_alt_old_val;
	newval = int( new_val );
    if ( newval == 1 )
    {
        self._active_glow_red = playfxontag( 0, level._effect["perk_machine_light_red"], self, "tag_origin" );
        self.artifact_glow_setting = 1;
...
turn_on_active_light_green_alt( new_val )
{

    if ( !isdefined( self.turn_on_active_light_green_alt_old_val ) )
	{
		self.turn_on_active_light_green_alt_old_val = 0;
	}
	oldval = self.turn_on_active_light_green_alt_old_val;
	newval = int( new_val );
    if ( newval == 1 )
    {
        self._active_glow_green = playfxontag( 0, level._effect["perk_machine_light_green"], self, "tag_origin" );
        self.artifact_glow_setting = 1;
...
turn_on_active_ball_light_alt( new_val )
{
    
    if ( !isdefined( self.turn_on_active_ball_light_alt_old_val ) )
	{
		self.turn_on_active_ball_light_alt_old_val = 0;
	}
	oldval = self.turn_on_active_ball_light_alt_old_val;
	newval = int( new_val );
    if ( newval == 1 )
    {
        self._ball_glow = playfxontag( 0, level._effect["perk_machine_light"], self, "j_ball" );
        self.artifact_glow_setting = 1;
...
// }
start_bottle_cycling_alt( new_val )
{
    if ( !isdefined( self.start_bottle_cycling_alt_old_val ) )
	{
		self.start_bottle_cycling_alt_old_val = 0;
	}
	oldval = self.start_bottle_cycling_alt_old_val;
	newval = int( new_val );
    if ( newval == 1 )
        self thread start_vortex_fx( 0 );
    else
        self thread stop_vortex_fx( 0 );
}
