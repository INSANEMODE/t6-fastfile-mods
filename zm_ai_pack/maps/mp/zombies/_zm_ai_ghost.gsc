...
ghost_think()
{
	...
	self.ignore_slowgun_anim_rates = undefined;
	if ( isDefined( level.slowgun_set_anim_rate_func ) )
	{
...
start_ghost_round()
{
	...
	if ( level flag_exists( "time_bomb_restore_active" ) && !flag( "time_bomb_restore_active" ) )
...
increase_ghost_health()
{
	if ( level.zombie_ghost_round_states.round_count == 1 )
	{
		...
		level.ghost_health = level.ghost_health + 500;
	else if ( level.zombie_ghost_round_states.round_count == 3 )
		level.ghost_health = level.ghost_health + 400;
...
{
	return level flag_exists( "time_bomb_restore_active" ) && ( !flag( "time_bomb_restore_active" ) || flag( "time_bomb_restore_active" ) && isDefined( level.time_bomb_get_time_bomb_saved_round_type ) && [[ level.time_bomb_get_time_bomb_saved_round_type ]]() == "ghost" );
}
...
#/
	...
	if ( level flag_exists( "time_bomb_round_killed" ) && level flag_exists( "time_bomb_enemies_restored" ) && flag( "time_bomb_round_killed" ) && !flag( "time_bomb_enemies_restored" ) )
		return false;
...
ghost_round_presentation_think()
{
	...
	while ( true )
	{
		if ( false && can_start_ghost_round_presentation() )
		{
...
init_time_bomb_ghost_rounds()
{
	if ( isDefined( level.time_bomb_register_time_bomb_enemy ) )
	{
	...
	{
		[[ level.time_bomb_time_bomb_add_custom_func_global_save ]]( ::time_bomb_global_data_save_ghosts );
	}
	if ( isDefined( level.time_bomb_add_custom_func_global_restore ) )
	{
		[[ level.time_bomb_add_custom_func_global_restore ]]( ::time_bomb_global_data_restore_ghosts );
	}
	level._time_bomb.custom_funcs_get_enemies = ::time_bomb_custom_get_enemy_func;
	if ( isDefined( level.register_time_bomb_enemy_save_filter ) )
	{
		[[ level.register_time_bomb_enemy_save_filter ]]( "zombie", ::is_ghost );
	}
}
...
_respawn_ghost_failsafe()
{
	...
	if ( !level flag_exists( "time_bomb_enemies_restored" ) )
	{
		return;
	}
